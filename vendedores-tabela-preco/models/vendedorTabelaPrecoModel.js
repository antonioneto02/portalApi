...
const SELECT_BASE = `
  ...
  LEFT JOIN p11_prod..SA3010 A3 ON A3.A3_COD = H02.H02_VEND AND A3.D_E_L_E_T_=''
  LEFT JOIN p11_prod..DA0010 DA0 ON DA0.DA0_CODTAB = H02.H02_CODTAB AND DA0.D_E_L_E_T_=''
`;
...
async function existeAssociacao(vend, codtab) {
  return !!(await (await getPool()).request()
    ...
    .input('vend', sql.VarChar(4), vend)
    .input('codtab', sql.VarChar(3), codtab)
    .query(`SELECT TOP 1 1 AS ok FROM p11_prod..H02010 WHERE H02_FILIAL=@filial AND H02_VEND=@vend AND H02_CODTAB=@codtab`)).recordset[0];
}
...
async function criar(dados) {
  ...
  await pool.request()
    ...
    .query(`INSERT INTO p11_prod..H02010 (H02_FILIAL, H02_VEND, H02_CODTAB, H02_DATAALT)
...
async function atualizar(vendAtual, codtabAtual, dados) {
  ...
  const result = await pool.request()
    ...
    .input('dataalt', sql.VarChar(8), dados.dataalt)
    .query(`UPDATE p11_prod..H02010 SET H02_VEND=@vendNovo, H02_CODTAB=@codtabNovo, H02_DATAALT=@dataalt
...
async function excluir(vend, codtab) {
  const result = await (await getPool()).request()
    ...
    .input('vend', sql.VarChar(4), vend)
    .input('codtab', sql.VarChar(3), codtab)
    .query(`DELETE FROM p11_prod..H02010 WHERE H02_FILIAL=@filial AND H02_VEND=@vend AND H02_CODTAB=@codtab`);
  return result.rowsAffected[0];
}
